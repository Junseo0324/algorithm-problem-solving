...
fun main() {
    ...
    val waiting = ArrayDeque(array)
    val bridge = ArrayDeque(List(w){0})
    var time = 0
    var totalWeight = 0
    while (true) {
        if (waiting.isEmpty() && bridge.all { it ==0 }) break
        if (bridge.all { it == 0 }) { // 다리가 비어있을 때
            val curr = waiting.removeFirst()
            ...
            if (waiting.isNotEmpty()) {
                ...
                if (totalWeight + curr > l) { //만약 waiting 중인 데이터가 나왔을 때 무게 초과한다면 다시 0을 넣음
                    bridge.addLast(0)
                } else {
